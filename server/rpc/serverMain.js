...
function handleJSON(isGraph, res, ss){
  var retrJSON;
  if (isGraph == false){
	 retrJSON = {"person":{"appUUID":"f5a91faa-e037-4ea1-83a4-2c21c04faafa", "clientUUID":"f6fa1db9-040f-4dd5-93ef-f5d9ee4b50cb"}, "UserAll":"true", "Emotions":emotionSearchFor, "email":"mdaiter@exeter.edu"};
  }
  else{
   retrJSON = {"person":{"appUUID":"f5a91faa-e037-4ea1-83a4-2c21c04faafa", "clientUUID":"f6fa1db9-040f-4dd5-93ef-f5d9ee4b50cb"}, "UserAll":"true", "AppName":appNameSearchedFor, "Emotions":emotionSearchFor, "email":"mdaiter@exeter.edu"};
  }
  ...
  request(options, function (error, response, body) {
    if (!error) {
      ...
      if (isGraph === false){
        ...
        processGraph(currEmotions, appNameSearchedFor, res, ss);
      }
    }
  });
}
...
function processGraph(json, nameToBeGraphed, res, ss){
        ...
        for (var i = dataArray.length - 1; i >= 0; i--) {
          ...
          for (var date in dataArray[i]){
            ...
            if (appName === nameToBeGraphed){
              ...
              var lastIndex = -1;
              lastIndex = sendDataContainsAppAndDate(nameToBeGraphed, date);
              if (lastIndex !== -1){
                console.log("Updated current stats");
                updateCurrAppStats(lastIndex, emoVal);
              }
              else{
                var lengthOfArray = sendData.data.length;
                sendData.data[lengthOfArray] = new Object();
                sendData.data[lengthOfArray].appName = appName;
                sendData.data[lengthOfArray].emotion = emoVal;
                sendData.data[lengthOfArray].numUsers = 1;
                sendData.data[lengthOfArray].date = new Date(date);

                console.log("Updated send data is: " + sendData.data[0].appName);
              }
            }
          }
        };        
  ...
  for (var i = sendData.data.length - 1; i >= 0; i--) {
    console.log("Done processing JSON data with name: " + sendData.data[i].appName);
    console.log("and emotion: " + sendData.data[i].emotion);
    console.log("and numUsers: " + sendData.data[i].numUsers);
    console.log("and date: " + sendData.data[i].date);
  };
  ss.publish.all('changeToGraph', sendData);     // Broadcast the message to everyone
  return res(true);
  ...
  for (var attrFirstLevel in json){
    ...
    if (typeof json[attrFirstLevel] === "object"){
      ...
      for (var attrSecLevel in attrFirstLevelSolid)
        ...
        var latestDateObject = dataArray[dataArray.length - 1];
...
exports.actions = function(req, res, ss) {
  ...
  return {
    ...
    updateGraph: function(valOfCheckButton, appName){
      ...
      ss.publish.all('newGraph', sendData);
    }
